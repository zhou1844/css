...
/* 腾讯电脑管家 */
var score_config = {
	1: ['数据', 'unknow', '//pc1.gtimg.com/pcmgr/online_server/images/webadmin/6.png'],
	2: ['数据', 'danger', '//pc1.gtimg.com/pcmgr/online_server/images/webadmin/3.png'],
	3: ['数据', 'safe', '//pc1.gtimg.com/pcmgr/online_server/images/webadmin/1.png'],
	4: ['数据', 'safe', '//pc1.gtimg.com/pcmgr/online_server/images/webadmin/1.png'],
	13: ['数据', 'low', '//pc1.gtimg.com/pcmgr/online_server/images/webadmin/2.png'],
	2001: ['数据', 'money', '//pc1.gtimg.com/pcmgr/online_server/images/webadmin/48_money.png']
};
	...
	converthtml : function() {
		...
		if(lastpage) {
			...
			for(var i=1; i<=lastpage; i++) {
				selector += '<option value="'+i+'" '+ (i == curpage ? 'selected' : '') +'>绗�'+i+'椤�</option>';
			}
			selector += '</select>';
			selector += '绗� '+curpage+' 椤�';
		}
		$('div.pg').removeClass('pg').addClass('comiis_page bg_f').html('<a href="'+ prevpagehref +'" class="bg_f b_ok">涓婁竴椤�</a>'+ selector +'<a href="'+ nextpagehref +'" class="bg_f b_ok">涓嬩竴椤�</a>');
...
var popup = {
	...
	open : function(pop, type, url) {
		...
		if(type == 'alert') {
			if(pop.indexOf("闈炲父鎰熻阿锛屾偍鐨勪富棰樺凡鍙戝竷") >= 0){
				pop = '鍙戝竷鎴愬姛';
			}
			...
			pop = pop.replace('[ 鐐瑰嚮杩欓噷杞叆涓婚鍒楄〃 ]', '');
		...
		if(typeof pop == 'string') {
			...
			if(type == 'alerts') {
				pop = '<div class="comiis_tip bg_f cl"><dt class="f_b"><p>'+ pop +'</p></dt><dd class="b_t cl"><a href="javascript:;" onclick="popup.close();" class="tip_btn tip_all bg_f f_b">纭畾</a></dd></div>'
			} else if(type == 'confirm') {			
			if(comiis_post_btnwz == 1){
				pop = '<div class="comiis_tip bg_f cl"><dt class="f_b"><p>'+ pop +'</p></dt><dd class="b_t cl"><a href="javascript:;" onclick="popup.close();" class="tip_btn bg_f f_b">鍙栨秷</a><a href="'+ url +'" class="tip_btn bg_f f_0"><span class="tip_lx">纭畾</span></a></dd></div>'
			}else{
				pop = '<div class="comiis_tip bg_f cl"><dt class="f_b"><p>'+ pop +'</p></dt><dd class="b_t cl"><a href="'+ url +'" class="tip_btn bg_f f_0">纭畾</a><a href="javascript:;" onclick="popup.close();" class="tip_btn bg_f f_b"><span class="tip_lx">鍙栨秷</span></a></dd></div>'
			}			
			}
...
var geo = {
	...
	locationerror : function(error) {
		...
		switch(error.code) {
			case error.TIMEOUT:
				geo.errmsg = "鑾峰彇浣嶇疆瓒呮椂锛岃閲嶈瘯";
				break;
			case error.POSITION_UNAVAILABLE:
				geo.errmsg = '鏃犳硶妫€娴嬪埌鎮ㄧ殑褰撳墠浣嶇疆';
			    break;
		    case error.PERMISSION_DENIED:
		        geo.errmsg = '璇峰厑璁歌兘澶熸甯歌闂偍鐨勫綋鍓嶄綅缃�';
		        break;
...
function comiis_getday(id, Year, Month, day, istime){
	...
	for(var k = 1; k <= d.getDate(); k++) {
		html += '<option value="' + ( k < 10 ? '0' : '') + k + '"' + (k == day ? 'selected="selected"' : '') + '>' + ( k < 10 ? '0' : '') + k + '鏃�</option>'; 
	}
...
function comiis_setdate(id, istime){
	...
	if(comiis_date_style == 1){
		...
		popup.open(title ? title + '宸叉洿鏀�' : '鏃ユ湡' + (this_id.hasClass("comiis_dateshow") ? '鏃堕棿' : '') + '宸叉洿鏀�', 'alert');
	}
...
$(document).on('click', '.comiis_dateshow:text,.comiis_dateshow_nt:text', function() {
	...
	'<div class="tip_tit bg_e f_b b_b">' + (title ? title : '鏃ユ湡'+(istime ? '鏃堕棿' : '')+'璁剧疆') + '</div>' + 
	...
	for(var k = newDate.getFullYear() + 5; k >= 2000 ; k--) {
		datehtml += '<option value="' + k + '"' + (k == comiis_y ? 'selected="selected"' : '') + '>' + k + '骞�</option>';
	}
	datehtml += '</select>' + 
	'</div>' + 
	...
	for(var k = 1; k <= 12; k++) {
		datehtml += '<option value="' + ( k < 10 ? '0' : '') + k + '"' + (k == comiis_m ? 'selected="selected"' : '') + '>' + ( k < 10 ? '0' : '') + k + '鏈�</option>';
	}
	datehtml += '</select>' + 
	'</div>' + 
	...
	if(istime){
		...
		'<div class="comiis_date comiis_date_txt">鏃堕棿锛�</div>' + 
		...
		for(var k = 0; k <= 23; k++) {
			datehtml += '<option value="' + ( k < 10 ? '0' : '') + k + '"' + (k == comiis_h ? 'selected="selected"' : '') + '>' + ( k < 10 ? '0' : '') + k + '鏃�</option>';
		}
		datehtml += '</select>' + 
		'</div>' + 
		...
		for(var k = 0; k <= 59; k++) {
			datehtml += '<option value="' + ( k < 10 ? '0' : '') + k + '"' + (k == comiis_i ? 'selected="selected"' : '') + '>' + ( k < 10 ? '0' : '') + k + '鍒�</option>';
		}
		datehtml += '</select>' + 
		'</div>' + 
		'</div>'; 
    ...
    if(comiis_post_btnwz == 1){
        datehtml += '</div>' + 
        '<dd class="b_t cl">' + 
        '<a href="javascript:;" onclick="comiis_closedate()" class="tip_btn bg_f f_b">鍙栨秷</a>' + 
        '<a href="javascript:;" onclick="comiis_setdate(\'' + id + '\', \'' + istime + '\');" class="tip_btn bg_f f_0"><span class="tip_lx">纭畾</span></a>' + 
        '</dd>' + 
        '</div>';
    }else{
        datehtml += '</div>' + 
        '<dd class="b_t cl">' + 
        '<a href="javascript:;" onclick="comiis_setdate(\'' + id + '\', \'' + istime + '\');" class="tip_btn bg_f f_0">纭畾</a>' + 
        '<a href="javascript:;" onclick="comiis_closedate()" class="tip_btn bg_f f_b"><span class="tip_lx">鍙栨秷</span></a>' + 
        '</dd>' + 
        '</div>';
    }
	popup.open(datehtml);
